...
function FirstPage() {
  ...
  const handleOpenMenu = (e) => {
    e.stopPropagation(); // Prevent the event from bubbling to the global listener
    setOpen(true);
  };

  const handleClickOutside = (e) => {
    if (navMobileRef.current && !navMobileRef.current.contains(e.target) && navRef.current && !navRef.current.contains(e.target)) {
      setOpen(false);
    }
  };
  ...
  return (
    <div className='firstPageCont'>
      <div className='navContMobile' ref={navMobileRef} onClick={handleOpenMenu} style={{ opacity: open ? '0' : '1' }}>
        <div style={{ right: open ? '-100%' : '0' }}>
          <MenuIcon />
        </div>
      </div>
      <div className='navCont' ref={navRef}>
